...
export interface AccountsProps {
  ...
  onDirectPayment?: (partyType: 'CUSTOMER' | 'SUPPLIER', partyId: string, method: string, amt: number, desc?: string) => Promise<void>;
  canReceive?: boolean;
  canPay?: boolean;
  canCash?: boolean;
...
export function Accounts({
  ...
  const handleOpenCash = async () => {
    ...
    } catch (error: any) { setCashError(error?.message || 'Falha ao abrir o caixa.'); }
  ...
  const handleCashMovement = async () => {
    ...
    try {
      ...
      await loadCashSessions();
    } catch (error: any) { setCashError(error?.message || 'Falha ao registar o movimento.'); }
    finally { setCashLoading(false); }
  };
  ...
  const handleCloseCash = async () => {
    ...
    try {
      ...
      setClosingAmount(''); setClosingNote('');
      await loadCashSessions();
    } catch (error: any) { setCashError(error?.message || 'Falha ao fechar o caixa.'); }
    finally { setCashLoading(false); }
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded border border-[#c3c6d1] bg-white p-4 dark:border-[#43474f] dark:bg-[#1f2325]">
        <div className="flex flex-wrap gap-2">
          {([
            ...
            <button key={key} onClick={() => setView(key)} className={`rounded px-4 py-2 text-xs font-black uppercase ${view === key ? 'bg-[#003366] text-white' : 'bg-[#e7e8e9] text-[#30343b]'}`}>
              {label}
            </button>
          ))}
        </div>
        {view !== 'cash' && <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
        ...
        <section className="space-y-4 rounded-xl border border-[#c3c6d1] bg-white p-4 shadow-sm dark:border-[#43474f] dark:bg-[#1f2325]">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#e1e3e8] pb-4 dark:border-[#43474f]">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-xs text-[#737780]">Abra o caixa no início do turno. No fecho, introduza apenas o dinheiro contado fisicamente; o valor esperado só é revelado depois de confirmar.</p>
            </div>
            <span className={`rounded-full px-3 py-1 text-xs font-black uppercase ${openSession ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-700'}`}>{openSession ? 'Caixa aberto' : 'Caixa fechado'}</span>
          </div>

          {cashError && <div className="rounded border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-800">{cashError}</div>}

          {!openSession ? (
            <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
              <label className="text-xs font-black uppercase text-[#737780]">Fundo inicial (MZN)
                <input type="number" min="0" step="0.01" value={openingAmount} onChange={(event) => setOpeningAmount(event.target.value)} placeholder="0,00" className="mt-1 w-full rounded border border-[#c3c6d1] bg-white px-3 py-2 text-base font-bold dark:border-[#43474f] dark:bg-[#282c2e]" />
              </label>
              <button disabled={cashLoading} onClick={handleOpenCash} className="rounded bg-[#006e25] px-5 py-2.5 text-sm font-black text-white disabled:opacity-50">Abrir caixa</button>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <div className="rounded-lg bg-[#f4f6f8] p-3 dark:bg-[#282c2e]"><span className="block text-[11px] font-black uppercase text-[#737780]">Aberto em</span><strong className="text-sm">{new Date(openSession.openedAt).toLocaleString('pt-MZ')}</strong></div>
                <div className="rounded-lg bg-[#f4f6f8] p-3 dark:bg-[#282c2e]"><span className="block text-[11px] font-black uppercase text-[#737780]">Fundo inicial</span><strong className="font-mono text-base">{formatMZN(openSession.openingAmount)}</strong></div>
                <div className="rounded-lg border border-dashed border-[#c3c6d1] p-3"><span className="block text-[11px] font-black uppercase text-[#737780]">Valor esperado</span><strong className="text-sm">Oculto até ao fecho</strong></div>
              </div>
              ...
              <div className="grid gap-3 rounded-lg border border-[#d6d9df] p-3 md:grid-cols-[170px_160px_1fr_auto] md:items-end dark:border-[#43474f]">
                <label className="text-xs font-black uppercase text-[#737780]">Movimento
                  <select value={movementType} onChange={(event) => setMovementType(event.target.value as typeof movementType)} className="mt-1 w-full rounded border border-[#c3c6d1] bg-white p-2 dark:border-[#43474f] dark:bg-[#282c2e]"><option value="REINFORCEMENT">Reforço</option><option value="WITHDRAWAL">Sangria</option></select>
                </label>
                <label className="text-xs font-black uppercase text-[#737780]">Valor (MZN)
                  <input type="number" min="0.01" step="0.01" value={movementAmount} onChange={(event) => setMovementAmount(event.target.value)} className="mt-1 w-full rounded border border-[#c3c6d1] bg-white p-2 dark:border-[#43474f] dark:bg-[#282c2e]" />
                </label>
                <label className="text-xs font-black uppercase text-[#737780]">Motivo / nota
                  <input value={movementNote} onChange={(event) => setMovementNote(event.target.value)} placeholder={movementType === 'WITHDRAWAL' ? 'Obrigatório para sangria' : 'Opcional'} className="mt-1 w-full rounded border border-[#c3c6d1] bg-white p-2 dark:border-[#43474f] dark:bg-[#282c2e]" />
                </label>
                <button disabled={cashLoading} onClick={handleCashMovement} className="rounded bg-[#003366] px-4 py-2 text-xs font-black text-white disabled:opacity-50">Registar</button>
              </div>
              ...
              <div className="rounded-lg border-2 border-[#003366] p-4 dark:border-[#6ca7e8]">
                ...
                <p className="mb-3 mt-1 text-xs text-[#737780]">Conte fisicamente notas e moedas. Não mostramos o valor calculado antes do fecho.</p>
                <div className="grid gap-3 md:grid-cols-[180px_1fr_auto] md:items-end">
                  <label className="text-xs font-black uppercase text-[#737780]">Dinheiro contado (MZN)
                    <input type="number" min="0" step="0.01" value={closingAmount} onChange={(event) => setClosingAmount(event.target.value)} className="mt-1 w-full rounded border border-[#c3c6d1] bg-white p-2 text-base font-bold dark:border-[#43474f] dark:bg-[#282c2e]" />
                  </label>
                  <label className="text-xs font-black uppercase text-[#737780]">Observação
                    <input value={closingNote} onChange={(event) => setClosingNote(event.target.value)} placeholder="Opcional" className="mt-1 w-full rounded border border-[#c3c6d1] bg-white p-2 dark:border-[#43474f] dark:bg-[#282c2e]" />
                  </label>
                  <button disabled={cashLoading || closingAmount === ''} onClick={handleCloseCash} className="rounded bg-[#ba1a1a] px-5 py-2.5 text-sm font-black text-white disabled:opacity-50">Fechar caixa</button>
                </div>
              </div>
            </div>
          )}
          ...
          {lastClosedSession && (
            <div className={`rounded-lg border p-4 ${(lastClosedSession.varianceAmount || 0) === 0 ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}`}>
              <span className="block text-xs font-black uppercase">Resultado do último fecho</span>
              <div className="mt-2 grid gap-2 sm:grid-cols-3"><div>Esperado<br/><strong>{formatMZN(lastClosedSession.expectedClosingAmount || 0)}</strong></div><div>Contado<br/><strong>{formatMZN(lastClosedSession.declaredClosingAmount || 0)}</strong></div><div>Diferença<br/><strong>{formatMZN(lastClosedSession.varianceAmount || 0)}</strong></div></div>
            </div>
          )}

          <div>
            <h3 className="mb-2 text-xs font-black uppercase text-[#737780]">Últimos turnos</h3>
            {cashLoading && cashSessions.length === 0 ? <p className="text-sm text-[#737780]">A carregar...</p> : <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="bg-[#f1f3f5] uppercase text-[#737780] dark:bg-[#282c2e]"><tr><th className="p-2">Abertura</th><th className="p-2">Estado</th><th className="p-2 text-right">Fundo</th><th className="p-2 text-right">Contado</th><th className="p-2 text-right">Diferença</th></tr></thead><tbody>{cashSessions.map((session) => <tr key={session.id} className="border-b border-[#e1e3e8] dark:border-[#43474f]"><td className="p-2">{new Date(session.openedAt).toLocaleString('pt-MZ')}</td><td className="p-2 font-bold">{session.status === 'OPEN' ? 'Aberto' : 'Fechado'}</td><td className="p-2 text-right font-mono">{formatMZN(session.openingAmount)}</td><td className="p-2 text-right font-mono">{session.status === 'CLOSED' ? formatMZN(session.declaredClosingAmount || 0) : '—'}</td><td className="p-2 text-right font-mono">{session.status === 'CLOSED' ? formatMZN(session.varianceAmount || 0) : '—'}</td></tr>)}</tbody></table></div>}
          </div>
        </section>
      )}
      ...
      {view !== 'cash' && <section className="overflow-hidden rounded border border-[#c3c6d1] bg-white shadow-sm dark:border-[#43474f] dark:bg-[#1f2325]">
